...
// --- Bed Status Utilities ---

export const hasAnyStatus = (bed: BedState): boolean => {
  if (bed.status === BedStatus.IDLE) return false;
  return !!(bed.isInjection || bed.isFluid || bed.isManual || bed.isESWT || bed.isTraction);
};

export const getStatusAreaStyles = (
  hasStatus: boolean,
  isDesktop: boolean,
  isPortrait: boolean
): string => {
  const padding = !isDesktop && isPortrait && hasStatus ? 'p-0' : 'p-1';
  ...
  if (!isDesktop) {
    if (hasStatus) {
      ...
      width += isPortrait ? 'min-w-[25px] ' : 'min-w-[21px] ';
    }
  } else {
  ...
  if (!isDesktop) {
    if (isPortrait) {
      ...
      transform = hasStatus ? '-translate-x-[11px]' : 'translate-x-[1px]';
    }
  } else {
