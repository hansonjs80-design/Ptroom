...
export const BedEditOverlay: React.FC<BedEditOverlayProps> = memo(({
  ...
  // 1. Keyboard Shortcut for Closing
  useKeyboardShortcut({
    onEscape: onClose
  });

  // 2. Navigation for Quick Add Items
  // total items = quickTreatments.length + 1 (for Custom Add button)
  const totalQuickItems = quickTreatments.length + 1;

  const handleQuickSelect = (index: number) => {
    // This will be called on Enter
    const gridElement = document.getElementById('bed-edit-quick-grid');
    if (gridElement) {
  ...
  const { selectedIndex, setSelectedIndex } = useListNavigation({
    itemCount: totalQuickItems,
    onSelect: handleQuickSelect,
    active: true // Active while overlay is open
  });
